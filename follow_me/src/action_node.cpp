...
void update() {
    ...
    if ( new_goal_to_reach && init_odom && init_obstacle )
        init_action();

    //we are performing a rotation and a translation
    if ( cond_goal && init_odom && init_obstacle )
    {
        perform_rotation();
        //perform_translation();//only for the 2nd release of "follow_me" behavior
        //combine_rotation_and_translation();//only for the 2nd release of "follow_me" behavior
        move_robot();
    }
...
void combine_rotation_and_translation()
{
    ...
    // coef_rotation + coef_translation = 1
    float coef_rotation;// = ...; 0 <= coef_rotation <= 1
    float coef_translation = 1 - coef_rotation;// 0 <= coef_translation <= 1
