...
public:
    ...
    datmo() {
        ...
        pub_datmo = n.advertise<geometry_msgs::Point>("goal_to_reach",
                                                      1);     // Preparing a topic to publish the goal to reach.

        pub_datmo_marker = n.advertise<visualization_msgs::Marker>("datmo",
...
// store all the hits of the laser in the background table
    ...
    void detect_motion() {
        ...
        for (int loop = 0; loop < nb_beams; loop++) {//loop over all the hits
            ...
            */
            if (!current_robot_moving && std::abs(background[loop] - r[loop]) > detection_threshold) {
                dynamic[loop] = true;
            }
    ...
    void perform_clustering() {
        ...
        for (int loop = 1; loop < nb_beams; loop++)//loop over all the hits
            ...
            if (distancePoints(current_scan[loop - 1], current_scan[loop]) > cluster_threshold) {
                ...
                //- cluster_size to store the size of the cluster ie, the euclidean distance between the first hit of the cluster and the last one
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
    ...
    void track_a_moving_person() {
        ...
        //tracking_mode = ...;//we check that the moving_person_tracked has been lost or not
        if (tracking_mode) {
