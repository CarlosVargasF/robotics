...
public:
    ...
    clustering() {
        ...
        pub_moving_persons_detector_marker = n.advertise<visualization_msgs::Marker>("clustering",
...
//            if euclidian DISTANCE between the previous hit and the current one is higher than "cluster_threshold"
            if (distancePoints(current_scan[loop - 1], current_scan[loop]) > cluster_threshold) {
                ...
                colors[nb_pts].a = 1.0;
                nb_pts++;
            } else if (loop == nb_beams - 1) {
                ROS_INFO("end pts: %i", nb_pts);
                ROS_INFO("cluster[%i]: hit[%i](%f, %f) -> hit[%i](%f, %f)", nb_cluster, start, current_scan[start].x, current_scan[start].y, loop, current_scan[loop].x, current_scan[loop].y);
